...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/02 18:00:44 by mes-salh          #+#    #+#             */
/*   Updated: 2025/02/14 22:17:07 by mes-salh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (data->vars.map_set
		&& map_bordered(data->md.temp_map, data->md.height))
	{
...
int	check_line(t_data *data)
{
	if (data->vars.map_set == 1 && !is_map_line(data->vars.line)
		...
		data->vars.tr = 1;
	if ((is_id(data->vars.line) && s_tex(data->textures, data->vars.line))
		|| data->vars.tr == 1)
		return (1);
...
int	parse_file(int fd, t_data *data)
{
	...
	if (valid_bound(data) || data->md.player_count > 1
		|| data->md.player_count == 0 || pars_surrounded_doors(data))
		return (1);
	return (init_doors(data), !(data->textures->count == 4
			&& data->vars.floor_set && data->vars.ceiling_set));
}
